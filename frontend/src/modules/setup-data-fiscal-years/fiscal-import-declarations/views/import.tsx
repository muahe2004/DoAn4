...
import {
  useGetFiscalImportDeclarations,
  type FiscalImportDeclarationListResponse,
} from "../apis/getFiscalImportDeclarations";
import {
  useGetFiscalImportDeclarationDetails,
  type FiscalImportDeclarationDetailListResponse,
} from "../apis/getFiscalImportDeclarationDetails";
import { STATUS_DISPLAY } from "../../../../utils/statusDisplay";
import { formatQuantity } from "../utils/unitConversion";
import type {
  IFiscalImportDeclarationResponse,
  IFiscalImportDeclarationDetailResponse,
} from "../types";
...
function SetupFiscalImportDeclarations() {
  ...
  const { data: fiscalImportDeclarationDetailsData } =
    useGetFiscalImportDeclarationDetails(ParamsDetail, {
        ...
        <Box className="fiscal-search-filters">
          <Box className="filter-left-side">
            ...
            <Box className="search-box-container">
              <SearchEngine
                placeholder="Số tờ khai nhập, bill, số hóa đơn..."
        ...
        <TableContainer className="custom-scrollbar primary-table-theme primary-table-container">
          <Table stickyHeader size="small">
            <TableHead className="fiscal-table-head">
              {viewMode === "list" ? (
                ...
                <TableRow>
                  ...
                  <TableCell
                    align="center"
                    sx={{ minWidth: 150, bgcolor: "#fff3e0" }}
                  >
                    CHI TIẾT SAI LỆCH
                  </TableCell>
                </TableRow>
            ...
            <TableBody className="primary-tbody">
              {viewMode === "list" ? (
                fiscalImportDeclarationsData?.data &&
                fiscalImportDeclarationsData.data.length > 0 ? (
                  fiscalImportDeclarationsData.data.map(
                    (
                      declaration: IFiscalImportDeclarationResponse,
                      index: number
                    ) => {
                      const statusKey =
                      ...
                      return (
                        <TableRow key={`${declaration.import_declaration_number}-${index}`}>
                          ...
                          <TableCell align="center">
                            {declaration.internal_code || "-"}
                          </TableCell>
                          <TableCell align="center">
                            {declaration.import_declaration_number}
                          </TableCell>
                          <TableCell align="center">
              ...
              ) : fiscalImportDeclarationDetailsData?.data &&
                fiscalImportDeclarationDetailsData.data.length > 0 ? (
                fiscalImportDeclarationDetailsData.data.map(
                  (
                    ...
                    const convertedAcctQty =
                      detail.converted_accounting_quantity || 0;
                    ...
                    if (variance === 0) {
                      ...
                      const percent = (
                        (variance / convertedDeclQty) *
                        100
                      ).toFixed(1);
                      varianceStatus = `↑ Nhập thừa ${formatQuantity(
                        variance
                      )} (${percent}%)`;
                      ...
                      const percent = (
                        (Math.abs(variance) / convertedDeclQty) *
                        100
                      ).toFixed(1);
                      varianceStatus = `↓ Nhập thiếu ${formatQuantity(
                        Math.abs(variance)
                      )} (${percent}%)`;
                    ...
                    return (
                      <TableRow key={`${detail.id} - ${index}`}>
                        ...
                        <TableCell align="center">
                          {detail.licence_date
                            ? new Date(detail.licence_date).toLocaleDateString(
                                "vi-VN"
                              )
                            : "-"}
                        </TableCell>
